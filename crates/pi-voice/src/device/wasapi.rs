...
// `device` and `enumerator` are held for their lifetime side effects: keeping
// the COM pointers alive for as long as the stream is open.
#[expect(dead_code, reason = "COM pointers held for their lifetime side effects")]
struct BaseStream {
